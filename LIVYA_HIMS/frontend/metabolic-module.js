...
(function(){
  ...
  async function getHimsSession(){
    ...
    }catch(e){console.warn('[LIVYA] Could not read HIMS auth session',e)}
    return null;
  }
  ...
  async function open(){
    ...
    if(!overlay){
      ...
      frame.addEventListener('load',async()=>{
        ...
        try{ frame.contentWindow.postMessage({type:'LIVYA_HIMS_SESSION_READY'},location.origin); }catch(e){}
      });
    }
    overlay.classList.add('show');
    if(!frame.src || !frame.src.endsWith('/'+APP_PATH)) frame.src=APP_PATH;
    else await syncFrameSession();
  }
  ...
  window.LIVYA_METABOLIC_MODULE={open,close,clearSession:clearFrameSession};
})();